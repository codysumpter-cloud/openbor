...
rem ============================================================
rem OpenBOR PAK64 Builder
rem
rem This script builds a .pak file from the "data" folder using
rem the PAK64 format.
rem
rem Use this version if your project is too large for the normal
rem legacy PACK format.
rem
rem To use:
...
echo.
echo Building PAK64 file...
echo.

borpak.exe -b -6 -d "%DATA_FOLDER%" "%OUTPUT_FILE%"

if errorlevel 1 (
    echo.
    echo PAK64 build failed.
    echo.
...
echo.
echo PAK64 build complete: %OUTPUT_FILE%
echo.
pause